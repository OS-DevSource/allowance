...
- Native-window frame sampling confirmed intermediate resize frames in both directions, between 206 and 410 points. The window now follows SwiftUI presentation heights using one 0.32-second animation; the main layout stays anchored.
...
- XCTest is unavailable in this local Command Line Tools installation. The equivalent portable runner works; full-Xcode XCTest passed in GitHub Actions for v0.1.0 and runs again for subsequent updates.
