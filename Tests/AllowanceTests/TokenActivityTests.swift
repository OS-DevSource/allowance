...
final class TokenActivityTests: XCTestCase {
    func testDailyHistoryPreservesZeroAndMissingDates() throws {
        ...
        XCTAssertEqual(days.map { TokenActivity.calendar.component(.weekday, from: $0.date) }, Array(1...7))
        XCTAssertEqual(days[4].tokens, 23571621)
        XCTAssertEqual(days[3].tokens, 0)
        XCTAssertNil(days[2].tokens)
        XCTAssertNil(activity.days(now: now.addingTimeInterval(120), timeZone: TokenActivity.calendar.timeZone)[5].tokens)
        XCTAssertEqual(activity.summary?.lifetimeTokens, 14876244643)
    }
    func testSundayWeekRolloverAndFutureDays() throws {
        ...
        XCTAssertEqual(activity.days(now: sunday, timeZone: TokenActivity.calendar.timeZone).first?.date, sunday)
        ...
        XCTAssertEqual(activity.days(now: saturday.addingTimeInterval(120), timeZone: TokenActivity.calendar.timeZone).first?.date,
                       formatter.date(from: "2026-09-20T00:00:00Z"))
    }
    func testLocalTodayAndDSTWeek() throws {
        ...
        XCTAssertEqual(days[4].tokens, 10)
        XCTAssertNil(days[5].tokens)
        let dstSunday = try XCTUnwrap(ISO8601DateFormatter().date(from: "2026-03-08T12:00:00Z"))
        let dstDays = activity.days(now: dstSunday, timeZone: chicago)
        XCTAssertEqual(dstDays.map { calendar.component(.weekday, from: $0.date) }, Array(1...7))
        XCTAssertEqual(dstDays[1].date.timeIntervalSince(dstDays[0].date), 23 * 3600)
    }
